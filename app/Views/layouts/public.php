...
<body class="bg-slate-50 text-slate-900 font-sans" x-data="{ mobileMenu: false, scrolled: false }" @scroll.window="scrolled = (window.pageYOffset > 20)">
    ...
    <?php if (get_setting('promo_enabled', '0') == '1'): ?>
        <div class="py-2.5 px-4 text-center text-white text-xs md:text-sm font-bold tracking-wide relative z-[60]" 
            ...
            <?php if ($promo_url = get_setting('promo_link_url')): ?>
                <a href="<?= $promo_url ?>" class="ml-2 underline hover:text-white/80 transition"><?= get_setting('promo_link_text', 'Cek Sekarang') ?></a>
            <?php endif; ?>
        </div>
    <?php endif; ?>
    ...
    <nav class="fixed top-0 left-0 w-full z-50 transition-all duration-300" 
        ...
        <div class="container mx-auto px-4 flex justify-between items-center">
            ...
            <div class="hidden md:flex items-center space-x-8 font-medium">
                <a href="<?= base_url() ?>" class="transition <?= url_is('/') ? 'text-primary-600 font-bold' : 'hover:text-primary-600' ?>">Beranda</a>
                <a href="<?= base_url('services') ?>" class="transition <?= url_is('services*') ? 'text-primary-600 font-bold' : 'hover:text-primary-600' ?>">Layanan</a>
                <a href="<?= base_url('portfolio') ?>" class="transition <?= url_is('portfolio*') ? 'text-primary-600 font-bold' : 'hover:text-primary-600' ?>">Portofolio</a>
                <a href="<?= base_url('blog') ?>" class="transition <?= url_is('blog*') ? 'text-primary-600 font-bold' : 'hover:text-primary-600' ?>">Blog</a>
                <a href="<?= base_url('produk') ?>" class="transition <?= url_is('produk*') ? 'text-primary-600 font-bold' : 'hover:text-primary-600' ?>">Produk</a>
                <a href="<?= base_url('kontak') ?>" class="bg-primary-600 text-white px-6 py-2.5 rounded-full hover:bg-primary-700 transition shadow-lg shadow-primary-200">
                    Mulai Project
                </a>
            </div>
        ...
        <div x-show="mobileMenu" x-cloak 
            ...
            <div class="flex flex-col space-y-4 font-medium">
                <a href="<?= base_url() ?>" class="p-2 rounded <?= url_is('/') ? 'bg-primary-50 text-primary-600 font-bold' : 'hover:bg-slate-50' ?>">Beranda</a>
                <a href="<?= base_url('services') ?>" class="p-2 rounded <?= url_is('services*') ? 'bg-primary-50 text-primary-600 font-bold' : 'hover:bg-slate-50' ?>">Layanan</a>
                <a href="<?= base_url('portfolio') ?>" class="p-2 rounded <?= url_is('portfolio*') ? 'bg-primary-50 text-primary-600 font-bold' : 'hover:bg-slate-50' ?>">Portofolio</a>
                <a href="<?= base_url('blog') ?>" class="p-2 rounded <?= url_is('blog*') ? 'bg-primary-50 text-primary-600 font-bold' : 'hover:bg-slate-50' ?>">Blog</a>
                <a href="<?= base_url('produk') ?>" class="p-2 rounded <?= url_is('produk*') ? 'bg-primary-50 text-primary-600 font-bold' : 'hover:bg-slate-50' ?>">Produk</a>
                <a href="<?= base_url('kontak') ?>" class="p-3 bg-primary-600 text-white text-center rounded-xl font-bold">Mulai Project</a>
            </div>
        </div>
    </nav>
        ...
        <div class="container mx-auto px-4">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-12 mb-16">
                <div class="col-span-1 md:col-span-1">
                    <a href="<?= base_url() ?>" class="mb-6 block">
                        ...
                        ?>
                        <?php if ($display_logo): ?>
                            <img src="<?= base_url($display_logo) ?>" alt="<?= get_setting('company_name') ?>" class="h-8 w-auto <?= $logo_dark ? '' : 'brightness-0 invert' ?>">
                        <?php else: ?>
                    ...
                    <div class="flex space-x-4 mt-6">
                        <?php if ($instagram = get_setting('instagram_url')): ?>
                            <a href="<?= $instagram ?>" target="_blank" class="w-10 h-10 bg-slate-800 rounded-lg flex items-center justify-center hover:bg-primary-600 transition-colors">
                        ...
                        <?php if ($facebook = get_setting('facebook_url')): ?>
                            <a href="<?= $facebook ?>" target="_blank" class="w-10 h-10 bg-slate-800 rounded-lg flex items-center justify-center hover:bg-primary-600 transition-colors">
                                <span class="sr-only">Facebook</span>
                                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M9 8h-3v4h3v12h5v-12h3.642l.358-4h-4v-1.667c0-.955.192-1.333 1.115-1.333h2.885v-5h-3.808c-3.596 0-5.192 1.583-5.192 4.615v3.385z"/></svg>
                            </a>
                        <?php endif; ?>
                        <?php if ($linkedin = get_setting('linkedin_url')): ?>
                            <a href="<?= $linkedin ?>" target="_blank" class="w-10 h-10 bg-slate-800 rounded-lg flex items-center justify-center hover:bg-primary-600 transition-colors">
                ...
                <div>
                    ...
                    <ul class="space-y-4">
                        ...
                        ?>
                            <li><a href="<?= base_url('services/'.$fs->slug) ?>" class="hover:text-white transition"><?= $fs->title ?></a></li>
                        <?php endforeach; ?>
                    </ul>
                </div>
                <div>
                    ...
                    <ul class="space-y-4">
                        <li><a href="<?= base_url() ?>#tentang" class="hover:text-white transition">Tentang Kami</a></li>
                        <li><a href="<?= base_url('portfolio') ?>" class="hover:text-white transition">Portofolio</a></li>
                ...
                <div>
                    ...
                    <form action="<?= base_url('newsletter/subscribe') ?>" method="POST" class="flex">
                        <input type="email" name="email" placeholder="Email" class="w-full px-4 py-2 bg-slate-800 border border-slate-700 rounded-l-xl focus:outline-none focus:border-primary-500 text-white text-sm" required>
                        <button type="submit" class="px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white rounded-r-xl transition">
            ...
            <div class="border-t border-slate-800 pt-8 flex flex-col md:flex-row justify-between items-center">
                ...
                <div class="flex flex-wrap gap-4 mt-4 md:mt-0">
                    <?php if ($instagram = get_setting('instagram_url')): ?>
                        <a href="<?= $instagram ?>" target="_blank" class="hover:text-white transition" title="Instagram">Instagram</a>
                    <?php endif; ?>
                    <?php if ($linkedin = get_setting('linkedin_url')): ?>
                        <a href="<?= $linkedin ?>" target="_blank" class="hover:text-white transition" title="LinkedIn">LinkedIn</a>
                    <?php endif; ?>
                    <?php if ($facebook = get_setting('facebook_url')): ?>
                        <a href="<?= $facebook ?>" target="_blank" class="hover:text-white transition" title="Facebook">Facebook</a>
                    <?php endif; ?>
                    <?php if ($twitter = get_setting('twitter_url')): ?>
                        <a href="<?= $twitter ?>" target="_blank" class="hover:text-white transition" title="Twitter">Twitter</a>
                    <?php endif; ?>
                    <?php if ($youtube = get_setting('youtube_url')): ?>
                        <a href="<?= $youtube ?>" target="_blank" class="hover:text-white transition" title="YouTube">YouTube</a>
                    <?php endif; ?>
                    <?php if ($github = get_setting('github_url')): ?>
                        <a href="<?= $github ?>" target="_blank" class="hover:text-white transition" title="GitHub">GitHub</a>
                    <?php endif; ?>
                </div>
            </div>
        </div>
    ...
    ?>
    <a href="<?= $wa_url ?>" target="_blank" class="fixed bottom-8 right-8 bg-green-500 text-white p-4 rounded-full shadow-2xl hover:bg-green-600 transition-all hover:scale-110 z-50">
